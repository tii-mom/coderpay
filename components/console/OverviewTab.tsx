...
export function OverviewTab({ state, onSwitchTab }: OverviewTabProps) {
  ...
  const onboardingSteps = [
    {
      title: '创建应用',
      desc: '配置应用名称、回调地址和返回地址，获取 App ID / App Secret。',
      action: '去创建',
      tab: 'apps',
      done: state.apps.length > 0,
    },
    {
      title: '添加安卓设备',
      desc: '生成 dev_ 绑定码，在 Android App 中输入并完成绑定。',
      action: '添加设备',
      tab: 'devices',
      done: state.devices.length > 0,
    },
    {
      title: '上传收款码',
      desc: '上传微信或支付宝收款码，并绑定到安卓监听设备。',
      action: '上传收款码',
      tab: 'codes',
      done: state.paymentCodes.length > 0,
    },
    {
      title: '创建测试订单',
      desc: '使用接口文档或控制台沙箱创建一笔小额测试订单。',
      action: '查看文档',
      tab: 'docs',
    ...
    {
      title: '确认回调结果',
      desc: '订单成功后检查商户 Webhook 是否收到 success。',
      action: '查看回调',
      tab: 'webhooks',
      done: state.webhookLogs.some(log => log.result === 'success'),
    },
  ];
  ...
  return (
    <div className="flex flex-col gap-8 text-left" id="overview-tab-panel">
      ...
      <div className="bg-cp-card border border-cp rounded-2xl p-5 sm:p-6 flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h3 className="text-base font-bold text-white">首次接入步骤</h3>
            <p className="text-xs text-slate-500 mt-1">按顺序完成下面 5 步，即可开始接收真实订单。</p>
          </div>
          <button
            onClick={() => onSwitchTab('docs')}
            className="w-fit px-3.5 py-2 rounded-xl bg-[#0B1020] hover:bg-[#151B2E] border border-[rgba(255,255,255,0.08)] text-xs text-slate-300 hover:text-white font-semibold flex items-center gap-1.5"
          >
            查看接口文档 <ArrowRight className="w-3.5 h-3.5" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {onboardingSteps.map((step, index) => (
            <button
              ...
              className="text-left rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#0B1020]/55 hover:bg-[#151B2E] hover:border-blue-500/30 p-4 transition-all min-h-36 flex flex-col justify-between"
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`text-[10px] font-bold ${step.done ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {step.done ? '已完成' : '待完成'}
                </span>
              </div>
              <div className="mt-4">
              ...
              <span className="mt-4 text-[11px] font-bold text-blue-400 flex items-center gap-1">
