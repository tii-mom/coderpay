...
export default function HomePage() {
  ...
  const plans = [
    ...
    },
    {
      ...
      techFee: '免费额度 ¥5,000 /月，高出部分仅扣 0.5% 服务费',
      features: [
        '独立接入应用上限 5 个',
        '支持挂载多台设备（负载均衡、切码）',
        '微信 + 支付宝多个固定金额付款码',
    ...
    },
    {
      ...
      price: '89',
      period: ' / 月',
      desc: '适合交易频繁的高黏性项目，不再计比扣除服务费。',
      techFee: '0% 交易佣金扣除，不扣任何技术服务费。',
      features: [
        '无限独立接入应用注册',
        '支持无限挂载备用安卓设备，高并发承载',
        '独享独立的异常离线邮件/短信提示服务',
  ...
  return (
    <div className="min-h-screen bg-[#070A12] text-slate-100 flex flex-col selection:bg-blue-500 selection:text-white" id="cp-root" suppressHydrationWarning>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center" id="hero-grid">
            ...
            <div className="lg:col-span-5" id="hero-right">
              <div className="relative p-6 sm:p-8 rounded-3xl bg-[#111827] border border-[rgba(255,255,255,0.08)] shadow-[0_0_30px_rgba(59,130,246,0.05)] text-left flex flex-col gap-6 overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/5 rounded-full blur-2xl" />
                
                <div className="flex items-center justify-between border-b border-[rgba(255,255,255,0.06)] pb-4">
                  <span className="text-xs text-blue-400 font-mono font-semibold tracking-wider block uppercase">PLATFORM PIPELINE</span>
                ...
                <div className="flex flex-col gap-4">
                  {flowSteps.map((step, idx) => (
                    ...
                    >
                      <div className="w-10 h-10 rounded-xl bg-blue-950 border border-blue-500/20 flex items-center justify-center text-sm font-bold font-mono text-blue-400">
                        {step.num}
                      </div>
      ...
      <section className="py-20 md:py-28 bg-[#0B1020] border-y border-[rgba(255,255,255,0.05)] relative" id="product-flow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="max-w-3xl mx-auto mb-16">
            <span className="text-xs text-blue-500 font-mono font-semibold tracking-widest uppercase block mb-3">PRODUCT PRINCIPLES</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left" id="capabilities-grid">
            {valueProps.map((prop, idx) => (
              <div key={idx} className="p-8 rounded-3xl bg-[#111827] border border-[rgba(255,255,255,0.06)] flex flex-col items-start gap-5">
      ...
      <section className="py-20 md:py-28" id="product-features">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-xs text-blue-500 font-mono font-semibold tracking-widest uppercase block mb-3">COMPARATIVE ADVANTAGE</span>
            <h2 className="text-3xl sm:text-4xl font-sans font-bold text-white tracking-tight mb-4">
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="bg-[#111827] border-b border-[rgba(255,255,255,0.08)]">
                  ...
                  <th className="py-5 px-6 text-sm font-semibold tracking-wider text-[#EF4444] font-sans w-3/8">传统商户 / 第三方易支付</th>
                  <th className="py-5 px-6 text-sm font-bold tracking-wider text-emerald-400 font-sans w-3/8 flex items-center gap-1.5">
                    Coder Pay <span className="text-[10px] uppercase font-mono px-1.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">WIN</span>
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[rgba(255,255,255,0.04)] bg-[#0B1020]">
                {comparePairs.map((pair, idx) => (
                  <tr key={idx} className="hover:bg-[#111827]/30 transition-colors">
                    ...
                    <td className="py-5 px-6 text-xs sm:text-sm text-gray-500 font-sans">{pair.traditional}</td>
      ...
      <section className="py-20 md:py-28 bg-[#0B1020] border-t border-[rgba(255,255,255,0.05)]" id="pricing-grid">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-xs text-blue-500 font-mono font-semibold tracking-widest uppercase block mb-3">AFFORDABLE PLANS</span>
            <h2 className="text-3xl sm:text-4xl font-sans font-bold text-white tracking-tight mb-4">
              透明计费套餐 · 低至单笔 1 分钱
            </h2>
            <p className="text-[#94A3B8] text-sm sm:text-base">
            ...
            {plans.map((p) => (
              <div
                ...
                className={`relative rounded-3xl p-8 flex flex-col justify-between transition-all duration-300 ${
                  p.primary
                    ? 'bg-[#111827] border-2 border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.15)] transform scale-[1.03] z-10'
                    : 'bg-[#111827] border border-[rgba(255,255,255,0.08)] hover:bg-[#151B2E] hover:border-gray-700'
                }`}
              >
                {p.primary && (
                  <span className="absolute top-0 right-8 -translate-y-1/2 bg-blue-600 border border-blue-400 text-white text-[10px] font-bold tracking-widest uppercase py-1 px-3.5 rounded-full shadow-lg">
                    POPULAR 极力推荐
                  </span>
                )}
