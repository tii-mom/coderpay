...
const MAX_BASE64_LENGTH = Math.ceil(MAX_FILE_SIZE * 4 / 3) + 128;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const fileType = String(body.fileType || body.file_type || "");
    ...
    if (!base64 || base64.length > MAX_BASE64_LENGTH) {
      return NextResponse.json({ error: "图片不能为空且不能超过 1MB" }, { status: 400 });
    }
    ...
    if (!hasExpectedMagic(bytes, fileType)) {
    ...
    console.error("Mobile payment code upload failed:", err);
