...
import { BILLING_PLANS, getNextSubscriptionExpiresAt, getSubscriptionChargeCents, PaidPackageType, TRIAL_PLAN } from "@/lib/billing-plans";
...
function isSubscribePlanId(value: string): value is PaidPackageType | "trial" {
  return value === "trial" || isPaidPackageType(value);
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const chargeCents = getSubscriptionChargeCents(planId, firstDiscountUsed);
    const charge = centsToBillingAmount(chargeCents);
